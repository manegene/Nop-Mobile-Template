...
namespace habahabamall.Behaviors.Catalog
{
    ...
    public class ListViewBehavior : Behavior<SfListView>
    {
        ...
        private async void Container_PropertyChanged(object sender, PropertyChangedEventArgs eventArgs)
        {
            if (eventArgs.PropertyName == "Height")
            {
                await Task.Delay(500);
                double extent = (double)this.container.GetType().GetRuntimeProperties()
