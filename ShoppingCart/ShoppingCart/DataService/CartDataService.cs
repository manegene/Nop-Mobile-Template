...
namespace habahabamall.DataService
{
    ...
    public class CartDataService : ICartDataService
    {
        ...
        public async Task<Status> AddOrderedItemAsync(int userId, int addressId, string paymentmode)
        {
            Status status = new Status();
            try
            {
                if (userId > 0 && !string.IsNullOrEmpty(paymentmode) && addressId>0)
                {
                    var orderobj = new
                    {
                        userid = userId,
                    ...
                    if (response != null)
                    {
                        ...
                        else if (result !=null && !response.IsSuccessStatusCode)
                        {
                                status = JsonConvert.DeserializeObject<Status>(result);
                        }
                    }
